                ...
                if (!empty($Uname) && !empty($Number) && !empty($Email)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            $sql="UPDATE users SET username=?, serialnumber=?, gender=?, email=?, user_date=CURDATE(), device_uid=?, device_dep=?, contact_number=?, add_card=1 WHERE id=?";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                ...
                                mysqli_stmt_bind_param($result, "sdssssssi", $Uname, $Number, $Gender, $Email, $dev_uid, $dev_name, $contact_number, $user_id );
                                mysqli_stmt_execute($result);
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!empty($Uname) && !empty($Email)) {
                                ...
                                $sql="UPDATE users SET username=?, serialnumber=?, gender=?, email=?, device_uid=?, contact_number=?, device_dep=? WHERE id=?";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    ...
                                    mysqli_stmt_bind_param($result, "sdsssssi", $Uname, $Number, $Gender, $Email, $dev_uid, $contact_number, $dev_name, $user_id );
                                    mysqli_stmt_execute($result);
